...
if len(sys.argv) != 3:
    print("Usage: $ python3 mdpVI.py <r> <gamma>")
    sys.exit()
...
while not convergence_achieved:
    ...
    for i in range(len(S)):
        if not is_terminal(S[i]):
            set_state_val(U, S[i], max(Q(S[i], 'up'), Q(S[i], 'down'), Q(S[i], 'left'), Q(S[i], 'right')))
            delta = max(delta, abs(get_state_val(U, S[i]) - get_state_val(U_old, S[i])))

    convergence_achieved = (delta <= epsilon)

print("\n\t # of iterations: " + str(num_iters))
print("┌─────────┬───────────────┬──────────┐")
...
for i in range(len(S)):
    if not is_terminal(S[i]):
        ...
        print("│  [{:d},{:d}]  │    {: .3f}     │  {:s}  │".format(S[i][0], S[i][1], get_state_val(U, S[i]), best_action))
